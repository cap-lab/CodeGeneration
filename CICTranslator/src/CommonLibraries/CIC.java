...
public class CIC {
	...
	public static void generateTaskCode(String mDestFile, String mTemplateFile, Task task, CICAlgorithmType mAlgorithm,
			CICControlType mControl) {
	...
	public static String translateTaskCode(String mContent, Task task, CICAlgorithmType mAlgorithm,
			CICControlType mControl) {
		...
		if (mAlgorithm.getLibraries() != null) {
			...
			for (TaskLibraryConnectionType taskLibCon : mAlgorithm.getLibraryConnections().getTaskLibraryConnection()) {
				if (taskLibCon.getMasterTask().equals(task.getName())) {
					...
					libraryDef += "#define LIBCALL_" + taskLibCon.getMasterPort() + "(f, ...) l_"
		...
		if (mtmFlag)
			includeHeaders += "#include \"cic_mtm_apis.h\"\n";
	...
	public static void generateTaskMTMCode(String mDestFile, String mTemplateFile, Task task,
			CICAlgorithmType mAlgorithm) {
		File fileOut = new File(mDestFile);
		File templateFile = new File(mTemplateFile);
		try {
			FileInputStream instream = new FileInputStream(templateFile);
	...
	public static void generateMTMFile(String outputPath, String mTemplateFile, Task task, CICAlgorithmType mAlgorithm,
			Map<String, Task> mTask, Map<String, Task> mPVTask, Map<Integer, Queue> mQueue,
			String runtimeExecutionPolicy) {
		...
		try {
			...
			outstream.write(translateMTMFile(outputPath, content, task, mAlgorithm, mTask, mPVTask, mQueue,
					runtimeExecutionPolicy).getBytes());
			outstream.close();
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
	...
	public static String translateMTMFile(String outputPath, String mContent, Task task, CICAlgorithmType mAlgorithm,
			Map<String, Task> mTask, Map<String, Task> mPVTask, Map<Integer, Queue> mQueue,
			String runtimeExecutionPolicy) {
		String code = mContent;
		String modeMap = "";
		...
		for (int i = 0; i < varList.size(); i++) {
			...
			if (type.equals("Integer")) {
				...
				transitionVarInit += "CIC_T_INT " + var.getName() + " = " + task.getName() + "_GetVariableInt(\""
						+ var.getName() + "\");";
			} else if (type.equals("String")) {
				stringVarMap += "\t{" + i + ", " + var.getName() + ", \"\"},\n";
				transitionVarInit += "CIC_T_CHAR* " + var.getName() + " = " + task.getName() + "_GetVariableString(\""
						+ var.getName() + "\");";
			}
		}
	...
	public static void generateTaskDataStructure(String mDestFile, String mTemplateFile, Map<String, Task> mTask,
			int globalPeriod, String globalPeriodMetric, String mThreadVer, String mRuntimeExecutionPolicy,
		...
		try {
			...
			outstream.write(translateTaskDataStructure(content, mTask, globalPeriod, globalPeriodMetric, mThreadVer,
					mRuntimeExecutionPolicy, mVTask, mPVTask).getBytes());
			outstream.close();
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
	...
	public static String translateTaskDataStructure(String code, Map<String, Task> mTask, int mGlobalPeriod,
			String mGlobalPeriodMetric, String mThreadVer, String mRuntimeExecutionPolicy, Map<String, Task> mVTask,
		...
		while (index < mTask.size()) {
			...
			if (parentTaskId == -1)
				parentTaskId = Integer.parseInt(task.getIndex());
			...
			taskEntriesCode += "\tENTRY(" + task.getIndex() + ", \"" + task.getName() + "\", " + taskType + ", "
					...
					+ Integer.toString(globalPeriod / Integer.parseInt(
							task.getPeriod()) /*
		...
		while (index < mTask.size() + mVTask.size() + mPVTask.size()) {
			...
			/*
			 * if(hasSubgraph.equals("CIC_V_FALSE")){
			 * if(!task.getName().equals(task.getParentTask())){ for(Task t:
			 * mVTask.values()){ if(task.getParentTask().equals(t.getName())){
			 * isChildTask = "CIC_V_TRUE"; parentTaskId =
			 * Integer.parseInt(t.getIndex()); break; } } } else{ isChildTask =
			 * "CIC_V_FALSE"; parentTaskId = index; } } else{ isChildTask =
			 * "CIC_V_FALSE"; parentTaskId = index; }
			 */
		...
		for (Task t : mTask.values()) {
			if (t.getHasMTM().equalsIgnoreCase("Yes")) {
				...
				mtmEntriesCode += "\tENTRY(" + t.getIndex() + ", " + isSDF + ", " + t.getName()
						+ ", CIC_V_MUTEX_INIT_INLINE, CIC_V_COND_INIT_INLINE),\n";
	...
	public static int getNextChannelIndex(int index, Map<Integer, Queue> mQueue,
			HashMap<String, ArrayList<Queue>> history) {
		...
		for (Queue queue : mQueue.values()) {
			...
			if (next_index != index) {
				if (target_queue.getSrc().equals(queue.getSrc())
						&& target_queue.getSrcPortId().equals(queue.getSrcPortId())) {
	...
	public static String translateChannelDataStructure(String mContent, Map<Integer, Queue> mQueue, String mThreadVer) {
		...
		for (Queue queue : mQueue.values()) {
			...
			String type = "";
			if (queue.getTypeName().equals("CHANNEL_TYPE_NORMAL"))
				type = "CIC_UT_CHANNEL_TYPE_NORMAL";
			else if (queue.getTypeName().equals("CHANNEL_TYPE_BUFFER"))
				type = "CIC_UT_CHANNEL_TYPE_BUFFER";
			else if (queue.getTypeName().equals("CHANNEL_TYPE_ARRAY_CHANNEL"))
				type = "CIC_UT_CHANNEL_TYPE_ARRAY_CHANNEL";

			channelEntriesCode += "\t{" + queue.getIndex() + ", " + nextChannelIndex + ", " + type
					+ ", CIC_V_NULL, CIC_V_NULL, CIC_V_NULL, " + queueSize + ", "
	...
	public static void generatePortmapHeader(String mDestFile, String mTemplateFile, Map<String, Task> mTask,
	...
	public static String translatePortmapDataStructure(String mContent, Map<String, Task> mTask,
			Map<Integer, Queue> mQueue) {
		String code = mContent;
	...
	public static void generateControlHeader(String mDestFile, String mTemplateFile, Map<String, Task> mTask,
			CICControlType mControl) {
		File fileOut = new File(mDestFile);
		File templateFile = new File(mTemplateFile);
		try {
	...
	public static String translateControlDataStructure(String mContent, Map<String, Task> mTask,
			CICControlType mControl) {
		String code = mContent;
		...
		if (mControl.getControlTasks() != null) {
			...
			// controlGroupCount =
			// Integer.toString(mControl.getExclusiveControlTasksList().getExclusiveControlTasks().size());
			controlGroupCount = "#define CIC_UV_CONTROL_GROUP_COUNT " + controlGroupCount;

			controlChannelCount = "1";
			// if(mControl.getControlTasks() != null) controlChannelCount =
			// Integer.toString(mControl.getControlTasks().getControlTask().size());
	...
	public static void generateCommonCode(String platform, String mOutputPath, String mTranslatorPath,
		...
		Util.copyFile(mOutputPath + "cic_control_apis.h",
				mTranslatorPath + "templates/common/common_template/cic_control_apis.h");

		// Copy cic_control.h
		Util.copyFile(mOutputPath + "cic_basic_control_apis.h",
				mTranslatorPath + "templates/common/common_template/cic_basic_control_apis.h");

		// Copy cic_mtm_apis.h file
		Util.copyFile(mOutputPath + "cic_mtm_apis.h",
				mTranslatorPath + "templates/common/common_template/cic_mtm_apis.h");
		...
		// Copy cic_channels.h
		Util.copyFile(mOutputPath + "cic_channels.h",
				mTranslatorPath + "templates/common/common_template/cic_channels.h");

		// Copy cic_portmap.h
		Util.copyFile(mOutputPath + "cic_portmap.h",
				mTranslatorPath + "templates/common/common_template/cic_portmap.h");

		// Copy cic_control.h
		Util.copyFile(mOutputPath + "cic_control.h",
				mTranslatorPath + "templates/common/common_template/cic_control.h");
