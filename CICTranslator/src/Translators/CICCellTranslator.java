...
public class CICCellTranslator implements CICTargetCodeTranslator {
	...
	private List<TaskLibraryConnectionType> mTaskLibConnection;		// List of taskLibraryconnection         (eliminated connection of same library)
	private List<LibraryLibraryConnectionType> mLibLibConnection;	// List of libraryLibraryconnection      (eliminated connection of same library)
	private List<String> mStubList;			// List of stub
	private List<String> mStubFileList;		// List of callee(stub) on PPE (not include wrapper)
	private List<String> mL_FileList;		// list of caller on PPE
	private List<String> mPPEToPPEList;		// List of ppe to ppe connection (not need channel & port)
...
//////////////////////////////////// Library ///////////////////////////////////////
		mTaskLibConnection = new ArrayList<TaskLibraryConnectionType>();// List of taskLibraryconnection      (eliminated connection of same library)
		mLibLibConnection = new ArrayList<LibraryLibraryConnectionType>();	// List of libraryLibararyconnection  (eliminated connection of same library)
		mStubList = new ArrayList<String>();			// List of stub
		mStubFileList = new ArrayList<String>();		// List of callee(stub) on PPE (not include wrapper)
		mL_FileList = new ArrayList<String>();		// list of caller on PPE
		mPPEToPPEList = new ArrayList<String>();		// List of ppe to ppe connection (not need channel & port)
...
////////////////////////////////////////////////////////////////////////////////////
	...
	public String translateCICInfoHeader(Task task){
		...
		int runCount = 0;
		if(task.getRunCondition().equals("TIME_DRIVEN"))	runCount = globalPeriod/Integer.parseInt(task.getPeriod());
		else												runCount = 0;
	...
	public void LibraryRelationGeneration(){
		for(TaskLibraryConnectionType taskLibCon: mAlgorithm.getLibraryConnections().getTaskLibraryConnection()){
			...
			if(taskLibConFlag == 0)	mTaskLibConnection.add(taskLibCon);
		}
		...
		for(LibraryLibraryConnectionType libLibCon: mAlgorithm.getLibraryConnections().getLibraryLibraryConnection()){
			...
			if(libLibConFlag == 0)	mLibLibConnection.add(libLibCon);
		}
		
		for(TaskLibraryConnectionType taskLibCon: mTaskLibConnection){
			int flag_1=0, flag_2=0, flag_3=0;
			for(Task task: mTaskOnSPE)
				if(taskLibCon.getMasterTask().equals(task.getName()))		flag_1 = 1;
			for(Library library: mLibraryOnSPE)
				if(taskLibCon.getSlaveLibrary().equals(library.getName()))	flag_2 = 1;
			
			if(flag_1 == 0 && flag_2 == 0)	mPPEToPPEList.add(taskLibCon.getSlaveLibrary());
			else if(flag_1 == 0 && flag_2 == 1){
				mL_FileList.add(taskLibCon.getSlaveLibrary());
				for(String stub: mStubList)
					if(stub.equals(taskLibCon.getSlaveLibrary()))	flag_3 = 1;
				if(flag_3==0)	mStubList.add(taskLibCon.getSlaveLibrary());
			}
			else{
				for(String stub: mStubList)
					if(stub.equals(taskLibCon.getSlaveLibrary()))	flag_3 = 1;
		...
		for(LibraryLibraryConnectionType libLibCon: mLibLibConnection){
			int flag_1=0, flag_2=0, flag_3=0;
			for(Library library: mLibraryOnSPE)
				if(libLibCon.getMasterLibrary().equals(library.getName()))	flag_1 = 1;
			for(Library library: mLibraryOnSPE)
				if(libLibCon.getSlaveLibrary().equals(library.getName()))	flag_2 = 1;
			
			if(flag_1 == 0 && flag_2 == 0)	mPPEToPPEList.add(libLibCon.getSlaveLibrary());
			else if(flag_1 == 0 && flag_2 == 1){
				mL_FileList.add(libLibCon.getSlaveLibrary());
				for(String stub: mStubList)
					if(stub.equals(libLibCon.getSlaveLibrary()))	flag_3 = 1;
				if(flag_3==0)	mStubList.add(libLibCon.getSlaveLibrary());
			}
			else{
				for(String stub: mStubList)
					if(stub.equals(libLibCon.getSlaveLibrary()))	flag_3 = 1;
		...
		if(mAlgorithm.getLibraries() != null){
			for(Library library: mLibrary.values()){
				int flag = 0;
				if(mLibraryOnSPE.contains(library)) continue;
				else{
					for(TaskLibraryConnectionType taskLibCon: mTaskLibConnection)
						if(taskLibCon.getSlaveLibrary().equals(library.getName()))
							for(Task task: mTaskOnSPE)
								if(task.getName().equals(taskLibCon.getMasterTask()))	flag = 1;
					for(LibraryLibraryConnectionType libLibCon: mLibLibConnection)
						if(libLibCon.getSlaveLibrary().equals(library.getName()))
							for(Library lib: mLibraryOnSPE)
								if(lib.getName().equals(libLibCon.getMasterLibrary()))	flag = 1;
					if(flag==1)	mStubFileList.add(library.getName());
				}
		...
		if(mAlgorithm.getLibraries() != null){
			for(Library library: mLibrary.values()) {
				...
				for(String ppe_ppe: mPPEToPPEList)
					if(ppe_ppe == library.getName())	flag = 1;
				if(flag == 1) continue;
				else{
					channelEntriesCode += "\t{\n\t\t" + library.getIndex() + ", 0, \n"
					...
					channelEntriesCode += "\t\t{CHANNEL_TYPE_ARRAY_CHANNEL, NULL, NULL, NULL, sizeof(" + library.getName() 
	...
	public String translateLibraryRefHeader(String mContent)
	{
		...
		if(mAlgorithm.getLibraries() != null){
			for(Library library: mLibrary.values()) {
				int flag = 0;
				for(String stub: mStubList)
					if(stub == library.getName())	flag = 1;
				if(flag == 0){
		...
		for(String ref: refList) {
			refEntriesCode += "ENTRY(" + Integer.toString(index) + ", \"" + ref + "\", l_" + ref + "_init, l_"
							+ ref + "_wrapup, 1),\n";
			index++;
		}
	...
	public String translateLibraryCalleeFile(Library library){
		...
		content += "\t\tLIB_RECEIVE(channel_id, 0, &receive_data, sizeof(" + library.getName() + "_func_data), index);\n";
		content += "\n\t\tfunc_index = receive_data.func_num;\n\n";
		content += "\t\tswitch(func_index) {\n";
		
		for(Function func: library.getFuncList()){
			content += "\t\t\tcase " + func.getIndex() + " : \n";
			if(func.getReturnType().equalsIgnoreCase("void"))	content += "\t\t\t\tLIBCALL(" + t_libCon + ", " + func.getFunctionName();
			else	content += "\t\t\t\tret_" + func.getFunctionName() + "= LIBCALL(" + t_libCon + ", " + func.getFunctionName();
			...
			content += "\t\t\t\tLIB_SEND(channel_id, 1, &send_data, sizeof(" + library.getName() + "_ret_data), index);\n";
			content += "\t\t\t\tbreak; \n\n";
		}
		content += "\t\t}\n";
		content += "\t}\n}\n\n";
	...
	public String translateLibraryCallerFile(Library library){
	    ...
	    for(TaskLibraryConnectionType taskLib: mAlgorithm.getLibraryConnections().getTaskLibraryConnection()){
	    	if(taskLib.getSlaveLibrary().equals(library.getName())){
	    		taskLibraryFlag = 1;
	    		for(Task task: mTaskOnSPE)
	    			if(taskLib.getMasterTask().equals(task.getName()))	flag = 1;
	    	}
	    }
	    for(LibraryLibraryConnectionType libLib: mAlgorithm.getLibraryConnections().getLibraryLibraryConnection()){
	    	if(libLib.getSlaveLibrary().equals(library.getName())){
	    		...
	    		for(Library subLib: mLibraryOnSPE)
	    			if(libLib.getMasterLibrary().equals(subLib.getName()))	flag = 1;
	    	}
	    }
	    ...
	    for(Function func: library.getFuncList()){
	    	...
	    	content += "\tLIB_RECEIVE(channel_id, 1, &receive_data, sizeof(" + library.getName() + "_ret_data), index);\n";
	...
	public String translateLibraryInfoHeader(Library library){
			...
			if(libLib.getMasterLibrary().equals(library.getName())){
				for(Library lib: mLibrary.values())
					if(libLib.getSlaveLibrary().equals(lib.getName()))
						libLibLibrary = lib;
				content += "\t{\n";
				content += "\t\t{0x0, " + libLibLibrary.getIndex() + ", MARS_TASK_QUEUE_MPU_TO_HOST, 0, sizeof(" + libLibLibrary.getName()
		        + "_func_data), 1},\n";
				content += "\t},\n";
				content += "\t{\n";
				content += "\t\t{0x0, " + libLibLibrary.getIndex() + ", MARS_TASK_QUEUE_HOST_TO_MPU, 1, sizeof(" + libLibLibrary.getName()
		                + "_ret_data), 1},\n";
				content += "\t},\n";
			}
		}
		...
		content += "\t{ 0x0, " + library.getIndex() + ", MARS_TASK_QUEUE_HOST_TO_MPU, 0, sizeof(" + library.getName() + "_func_data), 1},\n";
		content += "\t{ 0x0, " + library.getIndex() + ", MARS_TASK_QUEUE_MPU_TO_HOST, 1, sizeof(" + library.getName() + "_ret_data), 1},\n";
		...
		for(LibraryLibraryConnectionType libLib: mAlgorithm.getLibraryConnections().getLibraryLibraryConnection()){
			if(libLib.getMasterLibrary().equals(library.getName())){
				for(Library lib: mLibrary.values())
					if(libLib.getSlaveLibrary().equals(lib.getName()))
						libLibLibrary = lib;
				content += "\t{ 0x0, " + library.getIndex() + ", MARS_TASK_QUEUE_MPU_TO_HOST, 0, sizeof(" + library.getName() + "_func_data), 1},\n";
				content += "\t{ 0x0, " + library.getIndex() + ", MARS_TASK_QUEUE_HOST_TO_MPU, 1, sizeof(" + library.getName() + "_ret_data), 1},\n";
			}
		}
	...
	public String translateControlPortMapHeader(String mContent)
	{
		...
		for(Task task: mTask.values()){
			if(mTaskOnSPE.contains(task)){
				if(task.getIsSlaveTask()){
					controlPortMapCode += "\tENTRY(" + task.getIndex() + "/* " + task.getName() + "*/, \"" + task.getName()
	                   + "\", " + index + "),\n";
				}
			}
		}
	...
	public String translateControlInfoHeader(String mContent)
	{
		...
		if(mControl.getExclusiveControlTasksList() == null)
			...
			controlGroupCountCode += "#define CONTROL_GROUP_COUNT " + mControl.getExclusiveControlTasksList().getExclusiveControlTasks().size();
		
		if(mControl.getControlTasks() == null)
			controlChannelCountCode += "#define CONTROL_CHANNEL_COUNT 1";
		else
			controlChannelCountCode += "#define CONTROL_CHANNEL_COUNT " + mControl.getControlTasks().getControlTask().size();
		...
		if(mControl.getControlTasks() != null){
			...
			for(ControlTaskType controlTask: mControl.getControlTasks().getControlTask()){
				...
				for(ExclusiveControlTasksType exclusiveTasks: mControl.getExclusiveControlTasksList().getExclusiveControlTasks()){
				...
				controlChannelCode += "\t{" + taskId + ", " + controlTask.getPriority().intValue() + ", " + groupId + ", 0, 0, {0, }, {0, } },\n";
			}
		}
	...
	public void generateMakefile(String mDestFile)
	{
		...
		try {
		    ...
		    outstream.write("LDFLAGS=-m32 ./ppu/lib/libmars_task.a ./ppu/lib/libmars_base.a -lm -lX11 -lspe2 -lpthread -Xlinker --warn-common".getBytes());
		    ...
		    outstream.write("LDFLAGS_SPU=-Wl,--section-start,.init=0x4000 ./spu/lib/libmars_task.a ./spu/lib/libmars_base.a -Wl,-N -Wl,-gc-sections\n".getBytes());
		    ...
		    outstream.write(("proc.o: CIC_proc.c CIC_port.h cic_portmap.h cic_tasks.h cic_channels.h CON_port.h con_taskmap.h con_channels.h").getBytes());
		    ...
		    for(Task task: mTask.values()){
		    	if(mTaskOnSPE.contains(task)){
		    		if(!task.getIsSlaveTask()){
		    			outstream.write(("CIC_wrapper_ppe_" + task.getName() + ".o: " + task.getName() + ".info.c CIC_wrapper_ppe.c CIC_port.h cic_portmap.h").getBytes());
		    			for(String header: task.getExtraHeader())
		    				outstream.write((" " + header).getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("\t$(CC) $(CFLAGS) " + task.getCflag() + " -I./ppu/include -o CIC_wrapper_ppe_" + task.getName() + ".o -c CIC_wrapper_ppe.c"
		    					         + " -include " + task.getName() + ".info.c\n").getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("CIC_wrapper_spe_" + task.getName() + ".task_eo: CIC_port_spe.c " + task.getName() + ".c " + task.getCICFile() + " " 
		    					          + task.getName() + ".info.c " + "CIC_wrapper_spe.c CIC_port.h cic_portmap.h").getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) " + task.getCflag() + " -I./spu/include -o CIC_wrapper_spe_" + task.getName() + ".task_eo"
		    					         + " -c CIC_port_spe.c -include " + task.getName() + ".c -include " + task.getName() + ".info.c -include CIC_wrapper_spe.c").getBytes());
		    			for(String source: task.getExtraSource())
		    				outstream.write((" -include " + source).getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) -I./spu/include -o CIC_wrapper_spe_" + task.getName() + ".task"
   					                     + " CIC_wrapper_spe_" + task.getName() + ".task_eo $(LDFLAGS_SPU)\n").getBytes());
		    			outstream.write(("\t$(EMBEDSPU) -m32 CIC_wrapper_spe_" + task.getName() + "_prog CIC_wrapper_spe_" + task.getName() + ".task CIC_wrapper_spe_"
				                     + task.getName() + ".task_eo\n").getBytes());
		    			outstream.write(("\n").getBytes());
		    		}
		    		else if(task.getIsSlaveTask()){
		    			outstream.write(("CON_wrapper_ppe_" + task.getName() + ".o: " + task.getName() + ".info.c CON_wrapper_ppe.c CON_port.h CIC_port.h cic_portmap.h").getBytes());
		    			for(String header: task.getExtraHeader())
		    				outstream.write((" " + header).getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("\t$(CC) $(CFLAGS) " + task.getCflag() + " -I./ppu/include -o CON_wrapper_ppe_" + task.getName() + ".o -c CON_wrapper_ppe.c"
   					         + " -include " + task.getName() + ".info.c\n").getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("CON_wrapper_spe_" + task.getName() + ".task_eo: CON_port_spe.c " + task.getName() + ".c " + task.getCICFile() + " " 
		    					          + task.getName() + ".info.c " + "CON_wrapper_spe.c CIC_port.h cic_portmap.h CON_port.h con_portmap.h").getBytes());
		    			for(String header: task.getExtraHeader())
		    				outstream.write((" " + header).getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) " + task.getCflag() + " -I./spu/include -o CON_wrapper_spe_" + task.getName() + ".task_eo"
		    					         + " -c CON_port_spe.c -include " + task.getName() + ".c -include " + task.getName() + ".info.c -include CON_wrapper_spe.c").getBytes());
		    			for(String source: task.getExtraSource())
		    				outstream.write((" -include " + source).getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) -I./spu/include -o CON_wrapper_spe_" + task.getName() + ".task"
   					                     + " CON_wrapper_spe_" + task.getName() + ".task_o $(LDFLAGS_SPU)\n").getBytes());
		    			outstream.write(("\t$(EMBEDSPU) -m32 CON_wrapper_spe_" + task.getName() + "_prog CON_wrapper_spe_" + task.getName() + ".task CON_wrapper_spe_"
				                     + task.getName() + ".task_eo\n").getBytes());
		    			outstream.write(("\n").getBytes());
		    		}	
		    	}
		    		...
		    		for(String header: task.getExtraHeader())
	    				outstream.write((" " + header).getBytes());
		    		outstream.write(("\n").getBytes());
		    		outstream.write(("\t$(CC) $(CFLAGS) " + task.getCflag() + " -o " + task.getName() + ".o -c " + task.getName() + ".c\n\n").getBytes());
		    	}
		    }
		    ...
		    if(mAlgorithm.getLibraries() != null){
		    	...
		    	for(Library library: mLibrary.values()){
		    		...
		    		if(mLibraryOnSPE.contains(library)){
		    			outstream.write(("LIB_wrapper_ppe_" + library.getName() + ".o: " + library.getName() + ".info.c " + library.getName() 
		    					         + "_data_structure.h LIB_wrapper_ppe.c LIB_port.h lib_portmap.h\n").getBytes());
		    			outstream.write(("\t$(CC) $(CFLAGS) -I./ppu/include -o LIB_wrapper_ppe_" + library.getName() + ".o -c LIB_wrapper_ppe.c"
		    					         + " -include " + library.getName() + "_data_structure.h -include " + library.getName() + ".info.c\n").getBytes());
		    			outstream.write(("\n").getBytes());
		    			outstream.write(("LIB_wrapper_spe_" + library.getName() + ".task_eo: stub_" + library.getName()+ ".c " + library.getName() + ".info.c "
		    					         + library.getName() + "_data_structure.h " + library.getName() + ".c " + library.getName() + ".h " + library.getFile()
		    			...
		    			outstream.write(("\n").getBytes());
		    			for(String source: library.getExtraSource())
		    				outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) -c " + library.getName() + " -o " + library.getName() + ".o\n").getBytes());
		    			outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) -I./spu/include -o LIB_wrapper_spe_" + library.getName() + ".task_o"
		    					         + " -c LIB_port_spe.c -include " + library.getName() + "_data_structure.h -include stub_" + library.getName() + ".c -include " 
		    					         + library.getName() + ".info.c -include LIB_wrapper_spe.c -include " + library.getName() + ".c").getBytes());
		    			
		    			for(Library libLib: libLibList)
		    				outstream.write((" -include l_" + libLib.getName()).getBytes());
		    			outstream.write(("\n").getBytes());
		    			
		    			outstream.write(("\t$(CC_SPU) $(CFLAGS_SPU) -I./spu/include -o LIB_wrapper_spe_" + library.getName() + ".task"
		    					        + " LIB_wrapper_spe_" + library.getName() + ".task_o").getBytes());
		    			for(String extraSource: library.getExtraSource())
		    				outstream.write((" " + extraSource.substring(0, extraSource.length()-2)).getBytes());
		    			outstream.write((" $(LDFLAGS_SPU)\n").getBytes());
		    			outstream.write(("\t$(EMBEDSPU) -m32 LIB_wrapper_spe_" + library.getName() + "_prog LIB_wrapper_spe_" + library.getName() + ".task LIB_wrapper_spe_"
		    			...
		    			for(String stubFile: mStubFileList)
		    				if(stubFile.equals(library.getName()))	flag = 1;
		    			if(flag == 1){
		    				outstream.write(("stub_" + library.getName() + ".o: stub_" + library.getName() + ".c " + library.getName() + "_data_structure.h LIB_port.h\n").getBytes());
		    				outstream.write(("\t$(CC) $(CFLAGS) -o stub_" + library.getName() + ".o -c stub_" + library.getName() + ".c -include " + library.getName() + "_data_structure.h\n\n").getBytes());
		    			}
		    			outstream.write((library.getName() + ".o: " + library.getName() + ".c " + library.getFile() + " " + library.getHeader()+ "\n").getBytes());
		    			outstream.write(("\t$(CC) $(CFLAGS) -o " + library.getName() + ".o -c " + library.getName() + ".c\n\n").getBytes());
		    			...
		    			for(String extraSource: library.getExtraSource()){
			    			...
			    			for(String source: library.getExtraSource())
			    				outstream.write((" " + source).getBytes());
			    			outstream.write(("\n").getBytes());
			    			outstream.write(("\t$(CC) $(CFLAGS) -c " + extraSource + ".c -o " + extraSource.substring(0, extraSource.length()-2) + ".o\n\n").getBytes());
			    			outstream.write(("\n").getBytes());
		    			}
		    		}
		    	}
		    	for(String l_fileList: mL_FileList){
		    		outstream.write(("l_" + l_fileList + ".o: " + "l_" + l_fileList + ".c " + "l_" + l_fileList + ".h " + l_fileList + "_data_structure.h\n").getBytes());
		    		outstream.write(("\t$(CC) $(CFLAGS)").getBytes());
		    		outstream.write((" -c " + "l_" + l_fileList + ".c -o " + "l_" + l_fileList + ".o "+"-include "+"l_" + l_fileList + ".h " + "-include " + l_fileList +"_data_structure.h\n\n").getBytes());
		    	}
		    }
		...
		if(mAlgorithm.getLibraries() != null){
			...
			for(TaskLibraryConnectionType taskLibCon: mAlgorithm.getLibraryConnections().getTaskLibraryConnection()){
				if(taskLibCon.getMasterTask().equals(task.getName())){
					...
					content += "#define LIBCALL_" + taskLibCon.getMasterPort() + "(f, ...) l_" + taskLibCon.getSlaveLibrary() + "_##f(__VA_ARGS__)\n";
				}
			}
		}
