...
write_start:
    ...
    {
        if(gpuChannelInfo[channel_index].cpu_gpu != 2)        CUDA_ERROR_CHECK(cudaMemcpyAsync(gpuChannelInfo[channel_index].write_wait, buf, len, cudaMemcpyDeviceToHost, __stream[__previous_1_stream]));
        else if(gpuChannelInfo[channel_index].cpu_gpu == 2)   CUDA_ERROR_CHECK(cudaMemcpyAsync(gpuChannelInfo[channel_index].write_wait, buf, len, cudaMemcpyDeviceToDevice, __stream[__previous_1_stream]));
    ...
    else
    {
        ...
        if(part != 0)
        {
            if(gpuChannelInfo[channel_index].cpu_gpu != 2)        CUDA_ERROR_CHECK(cudaMemcpyAsync(gpuChannelInfo[channel_index].write_wait, buf, part, cudaMemcpyDeviceToHost, __stream[__previous_1_stream]));
            else if(gpuChannelInfo[channel_index].cpu_gpu == 2)   CUDA_ERROR_CHECK(cudaMemcpyAsync(gpuChannelInfo[channel_index].write_wait, buf, part, cudaMemcpyDeviceToDevice, __stream[__previous_1_stream]));
        }
        if(gpuChannelInfo[channel_index].cpu_gpu != 2)        CUDA_ERROR_CHECK(cudaMemcpyAsync(channel->buf, buf + part, len - part, cudaMemcpyDeviceToHost, __stream[__previous_1_stream]));
        else if(gpuChannelInfo[channel_index].cpu_gpu == 2)   CUDA_ERROR_CHECK(cudaMemcpyAsync(channel->buf, buf + part, len - part, cudaMemcpyDeviceToDevice, __stream[__previous_1_stream]));
...
TASK_POSTGO
{
    ...
    for(i=0; i<__num_of_channels; i++)
    {
        for(j=0; j<_num_portmaps; j++)
        {
            ...
            {  
                CUDA_ERROR_CHECK(cudaMemset((void*)__dev_buf[i][__previous_1_stream], 0x0, channels[addressmap[j].channel_id].sampleSize));
                break;
            }
        }
    }
