...
const uint32_t KC_const[16] = {0x9e3779b9UL,0x3c6ef373UL,0x78dde6e6UL,0xf1bbcdccUL,0xe3779b99UL,0xc6ef3733UL,0x8dde6e67UL,0x1bbcdccfUL,0x3779b99eUL,0x6ef3733cUL,0xdde6e678UL,0xbbcdccf1UL,0x779b99e3UL,0xef3733c6UL,0xde6e678dUL,0xbcdccf1bUL};
...
uem_result UCEncryptionSEED_GenerateRoundKey(uint8_t* userkey, uint8_t* roundkey)
{
...
#endif
	uint32_t *K = (uint32_t*)roundkey;     // Pointer of round keys

	// Set up input values for Key Schedule
	A = ((uint32_t *)userkey)[0];
	B = ((uint32_t *)userkey)[1];
	C = ((uint32_t *)userkey)[2];
	D = ((uint32_t *)userkey)[3];
	...
	for(j=0;j<7;j++){
		RoundKeyUpdate0(K+ i , A, B, C, D, KC_const[j*2] );     // K_1,0 and K_1,1
		RoundKeyUpdate1(K+ i +2, A, B, C, D, KC_const[j*2+1] ); // K_2,0 and K_2,1
		i = i + 4;
	}
	RoundKeyUpdate0(K+ 28 , A, B, C, D, KC_const[14] ); // K_1,0 and K_1,1
...
uem_result UCEncryptionSEED_Encode(uint8_t* roundkey, uint8_t* data)
{
	uem_result result = ERR_UEM_UNKNOWN;
	...
	for(j=0;j<8;j++){
		SEED_KeySched(L0, L1, R0, R1, K+ i);  
		SEED_KeySched(R0, R1, L0, L1, K+ i + 2);  
		i = i + 4;
	}
	...
	// Copying output values from last round to data
	((uint32_t *)data)[0] = R0;
	((uint32_t *)data)[1] = R1;
	((uint32_t *)data)[2] = L0;
	((uint32_t *)data)[3] = L1;
...
uem_result UCEncryptionSEED_Decode(uint8_t* roundkey, uint8_t* data)
{
	uem_result result = ERR_UEM_UNKNOWN;
	uint32_t L0, L1, R0, R1;   // Iuput/output values at each rounds
	uint32_t T0, T1;       // Temporary variables for round function F
	...
	// Copy output values from last round to data
	((uint32_t *)data)[0] = R0;
	((uint32_t *)data)[1] = R1;
	((uint32_t *)data)[2] = L0;
	((uint32_t *)data)[3] = L1;
...
uem_result UCEncryptionSEED_EncodeOnCTRMode(uint32_t *roundkey, uint8_t *iv, uem_uint8 *pData, uint32_t nDataLen)
{
	uem_result result = ERR_UEM_UNKNOWN;
	...
	for(i = 0; i < nBlockNum; i++) {
		...
		if(nBlockNum > 1) {
			...
			result = UCEncryptionSEED_Encode((uem_uint8*)roundkey, tiv);
