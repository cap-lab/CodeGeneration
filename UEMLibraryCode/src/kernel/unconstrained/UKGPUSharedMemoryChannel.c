...
uem_result UKGPUSharedMemoryChannel_Clear(SChannel *pstChannel)
{
	...
	result = setChunkNumAndLen(&(pstChannel->stInputPort), &(pstGPUSharedMemroyChannel->stInputPortChunk));
	ERRIFGOTO(result, _EXIT);

	result = setChunkNumAndLen(&(pstChannel->stOutputPort), &(pstGPUSharedMemroyChannel->stOutputPortChunk));
	ERRIFGOTO(result, _EXIT);

	pstGPUSharedMemroyChannel->nWrittenOutputChunkNum = CHUNK_NUM_NOT_INITIALIZED;
